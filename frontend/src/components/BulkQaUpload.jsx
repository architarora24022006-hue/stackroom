  ...
  return (
    <div className="card" style={{ padding: 18, marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
        ...
        <button className="btn-secondary" onClick={() => inputRef.current?.click()} disabled={processing} style={{ flexShrink: 0 }}>
          {processing ? 'Answering…' : 'Upload questions'}
        </button>
        <input ref={inputRef} type="file" accept=".xlsx,.xls" style={{ display: 'none' }} onChange={e => handleFile(e.target.files)} />
      </div>
      {error && <div className="error-banner" style={{ marginTop: 12 }}>{error}</div>}
      {done && !error && <div className="badge" style={{ marginTop: 12 }}>Answered file downloaded</div>}
    </div>
  )
}
